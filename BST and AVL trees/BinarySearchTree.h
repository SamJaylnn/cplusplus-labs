...
private:
    ...
    bool contains(const Comparable& data, BinaryNode<Comparable> *ptr);
};
...
BinarySearchTree<Comparable>::BinarySearchTree(const BinarySearchTree& rhs)
    : root(clone(rhs->root)),compare_count(rhs->compare_count),probe_count(rhs->probe_count)
{
}
...
/**
 * Public method to clear count;
 */
template <typename Comparable>
void BinarySearchTree<Comparable>::clear_count()
{
	compare_count = 0;
	probe_count = 0;
}
